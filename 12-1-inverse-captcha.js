...
/**
 * Executes the functions to test the teststring to the captcha solution function
 * @param  {Array}    testList - Array of tuples consisting of [teststring, answer]
 * @param  {Function} func     - Captcha solution function
 * @return {undefined}
 */
const execute = (testList, func) => {
  /**
   ...
   * @type {Array}
   */
  const results = testList.map((testString) => {
    const funcAnswer = func(testString[0]);
  ...
  console.log(func(captchaString));
};
...
const main = () => {
  execute(testStrings, inverseCaptcha);
  execute(testStrings2, inverseCaptcha2);
};
